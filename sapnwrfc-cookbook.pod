...
Note: refer to the README file in the package archive for build
instructions, and that kind of help.
...
The parameter L<debug> is an added one to trigger debugging messages in sapnwrfc.

B<Using SAP SSO>

To connect using SAP SSO, you can specify the ticket in Base64 encoded form, 
using the the L<mysapsso2> parameter. So not supply L<user> or L<passwd>.  SNC 
...
=item L<SAPNW::Rfc>, perl(1), ABAP(101).
